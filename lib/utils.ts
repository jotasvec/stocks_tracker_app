...
export function formatMarketCapValue(marketCapValue: number) : string{
  if (!Number.isFinite(marketCapValue) || marketCapValue <= 0) return 'N/A';

  if (marketCapValue >= 1e12) return `${(marketCapValue / 1e12).toFixed(2) }T`; // Trillions
  if (marketCapValue >= 1e9) return `${(marketCapValue / 1e9).toFixed(2) }B`; // Billions
  if (marketCapValue >= 1e6) return `${(marketCapValue / 1e6).toFixed(2) }M`; // Millions

  return `$${marketCapValue.toFixed(2)}`; // Below one million, show full USD amount
